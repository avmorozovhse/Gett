select r.country , r.city, r.week, invites, inviting_drivers, downloads, ftrs, riding_drivers, total_FTRs from (
-- Actives
    select city_name as city,
    o.Country_Symbol as country,
...
FROM dwh_fact_rides_v r, Dwh_Dim_Locations_v l WHERE r.origin_location_key = l.Location_Key GROUP BY driver_gk, city_name) l
ON i.Driver_GK = l.driver_gk AND ROW = 1
where i.Invite_Date_Key >= '2017-07-01'
group by city_name,
...
FROM dwh_fact_rides_v r, Dwh_Dim_Locations_v l WHERE r.origin_location_key = l.Location_Key  GROUP BY driver_gk, city_name) l
ON i.Driver_GK = l.driver_gk AND ROW = 1
where i.activated_at >= '2017-07-01'
group by city_name,
...
where  o.Is_FTP_Key = 1 and Order_Status_Key = 7  and o.date_key >= '2017-07-01'
group by city_name,
